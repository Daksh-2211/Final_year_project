    ...
    'margin-right': '10px'});
            ...
            if(profile_url != '' && /^[0-9]+$/.test(profile_url)) {
              ...
              $('#profile_url_error').show();
                ...
                if(profile_url != '' && profile_url.match(/^[-_][a-zA-Z0-9-_]*$/) || profile_url.match(/^[a-zA-Z0-9-_]*[-_]$/)){
                    ...
                    rpc("/profile/url/handler/vaidation",{'url_handler': profile_url})
    				.then(function (vals)
    				{
	    ...
	    $('#recently_product_tab').click(function(e){
	    	...
	    	if (temp === 1){
				...
				rpc("/seller/shop/recently-product/", {'shop_id': shop_id})
				.then(function (vals)
				{
		            var $modal = $(vals);
		            temp = temp+1;
	    ...
	    $('#profile_recently_product_tab').click(function(e){
			...
			if (temp === 1){
				$('.mp-box-review_loader').show();
                rpc("/seller/profile/recently-product/", {'seller_id': seller_id})
    			.then(function (vals)
    			{
    	            var $modal = $(vals);
                    temp = temp+1;
       	...
       	$('.sol-info').on('click', function()
        {
        	...
        	// var ajax = require('web.ajax');
        	var self = this;
			rpc("/track/sol", {'sol_id': order_line})
			.then(function (vals)
			{
				if (vals)
